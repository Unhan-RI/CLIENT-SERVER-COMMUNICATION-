...
if __name__ == "__main__":
    ...
    for i in range(num_nodes):
        node = Node(f"localhost", 20200 + i)
        node.start_server()
        nodes.append(node)

    # Example of adding files and searching
    for node in nodes:
        node.add_file("latihan.txt", "This is a sample file content.")
    ...
    for node in nodes:
        node.search_file("latihan.txt", ("localhost", 20200), use_random_walk=(search_method == "random walk"))
